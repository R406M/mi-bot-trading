...
def webhook():
    ...
    if action and amount > 0:  # Asegurarse de que los datos sean válidos
        try:
            ...
            try:
                if action == "buy":
                    # Comprar con todo el saldo USDT disponible
                    accounts = trade_client.get_accounts()  # Obtener todas las cuentas
                    usdt_balance = None
                    for account in accounts:
                        if account['currency'] == 'USDT' and account['type'] == 'trade':
                            usdt_balance = float(account['balance'])  # Obtener saldo de USDT
                            break
                    ...
                    if usdt_balance and usdt_balance > 0:
                        response = trade_client.create_market_order(
                            symbol=SYMBOL,
                            side=action,
                            funds=usdt_balance  # Usar todo el saldo USDT para comprar
                        )
                        print(f"Orden {action} realizada: {response}")
                    else:
                        print("No hay saldo suficiente de USDT")
                        return jsonify({"error": "Saldo insuficiente para realizar la compra"}), 400
                elif action == "sell":
                    # Vender todo el DOGE disponible
                    accounts = trade_client.get_accounts()  # Obtener todas las cuentas
                    doge_balance = None
                    for account in accounts:
                        if account['currency'] == 'DOGE' and account['type'] == 'trade':
                            doge_balance = float(account['balance'])  # Obtener saldo de DOGE
                            break
                    
                    if doge_balance and doge_balance > 0:
                        response = trade_client.create_market_order(
                            symbol=SYMBOL,
                            side=action,
                            size=doge_balance  # Vender todo el saldo DOGE
                        )
                        print(f"Orden {action} realizada: {response}")
                    else:
                        print("No hay DOGE disponible para vender")
